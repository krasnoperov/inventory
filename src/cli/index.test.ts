...
test('help command displays available commands', async () => {
  ...
  assert.ok(result.stdout.includes('upload <file> --collection <id>'), 'Help output should include upload collection placement');
  assert.ok(result.stdout.includes('upload <file> --manual-relation <spec>'), 'Help output should include upload manual relations');
  assert.ok(!result.stdout.includes('upload <manifest.json>'), 'Help output should not advertise manifest upload');
...
test('local help exits without toggling TLS warnings', async () => {
  ...
  try {
    ...
    assert.ok(result.stdout.includes('--collection <ids>'));
    assert.ok(result.stdout.includes('--manual-relation <spec>'));
