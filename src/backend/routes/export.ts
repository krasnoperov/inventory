...
function getAuthToken(cookieHeader: string | null): string | null {
  ...
  try {
    ...
    const manifest: ExportManifest = {
      ...
      lineage: (state.lineage || []).map(l => ({
        ...
        relationType: l.relation_type as 'derived' | 'composed',
      })),
    };
    ...
    return c.json({ error: 'Failed to export space' }, 500);
  }
});
