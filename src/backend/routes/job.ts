...
jobRoutes.post('/api/spaces/:spaceId/assets', generationRateLimiter, async (c) => {
  ...
  const { name, type, parentAssetId, prompt, referenceVariantIds: explicitVariantIds, referenceAssetIds, model, aspectRatio } = body;

  // We'll resolve referenceAssetIds to referenceVariantIds later if needed
  let referenceVariantIds = explicitVariantIds;
  ...
  // Determine operation mode based on prompt and references
  const hasPrompt = prompt !== undefined && prompt !== null;
  const hasExplicitVariantRefs = Array.isArray(referenceVariantIds) && referenceVariantIds.length > 0;
  const hasAssetRefs = Array.isArray(referenceAssetIds) && referenceAssetIds.length > 0;
  const hasRefs = hasExplicitVariantRefs || hasAssetRefs;

  // Validation based on operation mode
  if (!hasPrompt && !hasRefs) {
    return c.json({
      error: 'Either prompt or referenceVariantIds/referenceAssetIds is required. Provide prompt for AI generation, or references to fork from an existing variant.'
    }, 400);
  }

  // Get DO stub early - we need it for asset ID resolution
  if (!env.SPACES_DO) {
  ...
  // Resolve referenceAssetIds to referenceVariantIds if needed
  if (hasAssetRefs && !hasExplicitVariantRefs) {
    ...
    for (const assetId of referenceAssetIds) {
      ...
      if (!asset) {
        return c.json({ error: `Reference asset not found: ${assetId}` }, 404);
      }
      if (!asset.active_variant_id) {
        return c.json({ error: `Reference asset has no variant: ${asset.name}` }, 400);
      }
      resolvedVariantIds.push(asset.active_variant_id);
    }
    referenceVariantIds = resolvedVariantIds;
  }

  const refCount = Array.isArray(referenceVariantIds) ? referenceVariantIds.length : 0;

  if (!hasPrompt && refCount > 1) {
    return c.json({
      error: 'Cannot fork from multiple variants without a prompt. Provide a prompt to compose them, or use a single reference to fork.'
    }, 400);
  }
  ...
  const preCheck = await usageService.preCheck(userId, 'nanobanana');
  if (!preCheck.allowed) {
  ...
  usageService.incrementRateLimit(userId).catch(err =>
 ...
 * Request body:
 * {
 *   sourceVariantId?: string;       // Optional: variant to derive from (defaults to asset's active variant)
 *   prompt: string;                 // Required: modification instructions
 *   referenceAssetIds?: string[];   // Optional: asset IDs for additional references (resolved to default variants)
 *   referenceVariantIds?: string[]; // Optional: explicit variant IDs for additional references
 *   model?: string;                 // Optional: AI model
...
jobRoutes.post('/api/spaces/:spaceId/assets/:assetId/variants', generationRateLimiter, async (c) => {
  ...
  // Parse and validate request body
  const body = await c.req.json();
  const { sourceVariantId: explicitSourceVariantId, prompt, referenceVariantIds: explicitRefVariantIds, referenceAssetIds, model, aspectRatio } = body;

  // sourceVariantId is optional - will be resolved from asset's active_variant_id if not provided
  let sourceVariantId = explicitSourceVariantId;
  let referenceVariantIds = explicitRefVariantIds;
  ...
  // Pre-check: quota + rate limit before queueing image generation
  const usageService = container.get(UsageService);
  const variantPreCheck = await usageService.preCheck(userId, 'nanobanana');
  ...
  const assetData = await doResponse.json() as {
    asset: { id: string; name: string; type: string; active_variant_id: string | null };
    variants: Array<{ id: string; image_key: string }>;
  };

  // Resolve sourceVariantId from asset's active variant if not explicitly provided
  if (!sourceVariantId) {
    if (!assetData.asset.active_variant_id) {
      return c.json({ error: 'Asset has no active variant and no sourceVariantId was provided' }, 400);
    }
    sourceVariantId = assetData.asset.active_variant_id;
  }
  ...
  // Resolve referenceAssetIds to referenceVariantIds if provided
  const hasAssetRefs = Array.isArray(referenceAssetIds) && referenceAssetIds.length > 0;
  const hasExplicitRefVariants = Array.isArray(referenceVariantIds) && referenceVariantIds.length > 0;

  // Type for space state (needed for reference resolution)
  type SpaceStateType = {
    assets: Array<{ id: string; name: string; active_variant_id: string | null }>;
    variants: Array<{ id: string; image_key: string }>;
  };

  // Fetch full space state for reference resolution
  let spaceState: SpaceStateType | null = null;

  if (hasAssetRefs || hasExplicitRefVariants) {
    const stateResponse = await doStub.fetch(new Request('http://do/internal/state'));
    if (!stateResponse.ok) {
      return c.json({ error: 'Failed to fetch space state' }, 500);
    }
    spaceState = await stateResponse.json() as SpaceStateType;
  }

  // Resolve asset IDs to variant IDs if needed
  if (hasAssetRefs && !hasExplicitRefVariants && spaceState) {
    const resolvedVariantIds: string[] = [];
    for (const refAssetId of referenceAssetIds) {
      const refAsset = spaceState.assets.find(a => a.id === refAssetId);
      if (!refAsset) {
        return c.json({ error: `Reference asset not found: ${refAssetId}` }, 404);
      }
      if (!refAsset.active_variant_id) {
        return c.json({ error: `Reference asset has no variant: ${refAsset.name}` }, 400);
      }
      resolvedVariantIds.push(refAsset.active_variant_id);
    }
    referenceVariantIds = resolvedVariantIds;
  }
  ...
  if (Array.isArray(referenceVariantIds) && referenceVariantIds.length > 0 && spaceState) {
    for (const refId of referenceVariantIds) {
      const variant = spaceState.variants.find(v => v.id === refId);
      if (!variant) {
        return c.json({ error: `Reference variant not found: ${refId}` }, 404);
      }
