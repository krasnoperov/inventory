...
`videoResolution`, `videoDurationSeconds`, and per-request `generateAudio`
boolean in the stored recipe. Omitted `generateAudio` values are treated as
`false` by the workflow, preserving the silent default.
