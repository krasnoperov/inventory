...
- `media_kind` does not select a provider by itself. Audio generation enters through the website-controlled SpaceDO workflow lifecycle and uses `INVENTORY_AUDIO_PROVIDER` (`fake` or `elevenlabs`) plus server-side provider secrets/configuration. The ElevenLabs provider routes `music` assets to music generation, `sfx` assets to sound-effect generation, and other audio asset types to speech/dialogue generation. Future Google video providers should use the same lifecycle, set `mediaKind` explicitly, and choose the capable provider/model through generation provider/model fields.
